...
for(const site of [null,...sites.filter(s=>s.id==='cave:0:mouth:0'||s.id==='cave:0:gallery:0:0.4')]) {
 ...
 const frames=[];
 for(let i=0;i<600;i++) {if(process.argv[4]==='chase' && i>30 && i<300){const dx=model.listener.x-c.pos.x,dz=model.listener.z-c.pos.z,d=Math.hypot(dx,dz)||1;model.listener={x:c.pos.x+dx/d*18,y:c.ground+11,z:c.pos.z+dz/d*18};}calls=0;const t=performance.now();model.step(1/30);frames.push({time:model.time,ms:performance.now()-t,calls,states:g.members.map(c=>c.pebble.state)});}
 assert.ok(Math.max(...frames.map(f=>f.calls))<g.members.length*700+200,'Local steering exceeded its terrain-query budget');
 const sorted=frames.map(f=>f.ms).sort((a,b)=>a-b);
 reports.push({id:site?.id||'arrival',count:g.members.length,p95:sorted[Math.floor(sorted.length*.95)],max:sorted.at(-1),total:sorted.reduce((a,b)=>a+b,0),maxSamples:Math.max(...frames.map(f=>f.calls)),samples:frames.reduce((n,f)=>n+f.calls,0),worst:frames.sort((a,b)=>b.ms-a.ms).slice(0,6)});
}
