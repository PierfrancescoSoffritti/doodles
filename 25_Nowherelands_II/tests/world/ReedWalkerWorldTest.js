...
import { createReedFamily, reedHabitat, reedLocalPoint, REED_FAMILY_CAP } from '../../js/world/fauna/ReedWalkerHabitat.js';
import { reedJoint } from '../../js/world/fauna/ReedWalkerMotion.js';
const sample=(x,z)=>({ground:10+Math.sin(x*.02)*.15+Math.sin(z*.02)*.12,water:11,slope:.03,foam:.01,roof:false});
...
test('whole families spawn only on safe freshwater ground, and stream within a cap',()=>{
 for(const s of [{ground:0,water:0},{ground:0,water:8},{ground:10,water:11,roof:true},{ground:10,water:11,foam:.8}])assert.equal(reedHabitat(s),false);
...
test('terrain contacts remain planted through resting, grazing and complete wandering strides',()=>{
 ...
 for(let frame=0;frame<60*200;frame++){
  ...
  for(const g of model.groups.values())for(const m of g.members){
   ...
   if(pose.graze===1){feeding++;assert.ok(Math.abs((origin.y+(pose.body[1]-.83)*m.scale)-m.water)<1e-8);}
