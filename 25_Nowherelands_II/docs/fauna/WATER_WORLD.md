...
Where the entire swimming area is more than four units deep, fish spread through different depths and slowly rise and dive independently. The depth range grows with available water, up to 18 additional units, while shallow pools retain their near-surface routes. The shallowest bed across the validated disk limits every dive; escape checks include the deepest possible position and full body clearance. Separation accounts for vertical distance so fish can pass above and below one another. Depth profiles are seeded and resume at the same phase after streaming, with no extra terrain sampling per frame or rendering passes.
