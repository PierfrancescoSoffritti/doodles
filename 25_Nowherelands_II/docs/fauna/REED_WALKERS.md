...
`WorldReedWalkers` streams up to three families around the player and releases distant rigs and geometry. Candidates come from real rivers and lake margins. A whole family must fit safe, shallow freshwater terrain with no cave roof, strong foam or blocking collider; invalid sites are skipped. Game scale is 2.2 times atelier scale to suit the world’s player and vegetation proportions. The approved family count, randomized arrangement, sex/age colors, eyes and tusks are reused directly.

Each support pad uses sampled riverbed height. Slow four-foot stepping interpolates individual contacts, keeps supporting feet fixed, and permits small turns. The full movement is checked before committing to a step. Every member returns to grazing and rest, stays near its familiar patch, emits spatial sounds and produces water ripples while feeding. Underground streaming hides the surface population and suppresses its sounds.
...
World validation: terrain-contact unit tests cover 200 seconds of grazing/rest/turning/walking, family population limits and invalid habitats. `node 25_Nowherelands_II/tests/reed-world-audit.mjs` validates three minutes of movement in generated umbra and vesper terrain. Live browser inspection confirmed families in the full game with no runtime errors.
