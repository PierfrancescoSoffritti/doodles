...
## Entrance discoverability — 2026-09-19

The first cave now tries nearby suitable reaches of the starting river before falling back to other nearby river sites. Remaining caves retain the geological ranking and spacing rules. This preference cannot guarantee a cave on the starting river in every seed: the passage and roof-cover checks still apply. Mountain entrances also require a dry, descending approach so a shaped apron does not cross a protected stream.

Spring and broad mountain mouths are wider and taller, tapering into their original passage sizes; fissures and high entrances retain narrower proportions. Every entrance now has a broad, irregular exposed-limestone mask, including springs. Unequal buried bedrock ribs frame the mouth. Canopy clearance is baked into the shared habitat map so distant trees, nearby trees and terrain agree, and grasses/shrubs thin over the exposed rock. Wetness, coast and altitude channels are preserved. The landmark masks do not change river levels or terrain height.

Dry entrance aprons extend to about 200 world units. Their central grade connects the outer ground to the cave floor without extrapolating the descending underground floor into a convex lip. Terrain, cave collision and the meshed entrance still share the same generated data. Added **Distant approach** and **Side approach** buttons in `?caves=1` make inspection at walking height repeatable.

Browser inspection also exposed an existing cave-stream failure when the main render target had a depth texture: `WaterOptics` assumed all consumers supplied depth-reconstruction uniforms. Color-only cave streams now skip that optional depth copy. A regression test covers both depth-enabled and color-only targets.

Validation: 25 cave, cave-floor/fauna-habitat and water-optics tests, including deterministic generation, roof cover, water direction, both-way walking from 60 and 180 units on generated mountain/fissure approaches, vegetation clearing, and preservation of non-forest habitat channels. Desktop browser checks cover spring, mountain and fissure approaches. This pass changes cave locations for existing seeds; it does not add musical guidance. Mobile performance has not been remeasured.
