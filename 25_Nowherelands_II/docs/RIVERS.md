...
`WatersideFeatures.js` plans large wood once against the runtime heightmap, before the water caches obstacles. Fallen trees recruit in groups of up to three on either bank, requiring a wooded root site and dry, gently sloping woodland behind it. Sizes range from smaller bank trees to mature inland trunks, independently of channel width; larger trees can span the river. Tapered trunks have supported bank ends, varied broken and forked limbs, and root wads oriented across the trunk. Terrain probes follow the full length. Only immersed sections create flow obstacles; a trunk bridging above the water produces no artificial wake. Mixed broken wood collects upstream of immersed sections. Drowned standing trunks occupy sheltered shallow lake margins. Root fans and small soil caps project from eroded banks. The world plan is spatially indexed; chunk reloads cannot duplicate current obstacles.
