...
import { FaunaModel, SPECIES } from './FaunaModel.js';
import { FaunaMeshes } from './FaunaMeshes.js';
...
export class Fauna {
	constructor(scene, heightmap, shared) {
		...
		this.sample = (x, z) => {
			const ground = heightmap.sample(x, z), water = heightmap._water, slope = heightmap._slope || 0, foam = heightmap._foam || 0;
			const hab = heightmap.habitat(x, z);
			return { ground, water, slope, foam, forest: hab.forest, wet: hab.wet, coast: hab.coast };
		};
		this.obstacles = [];
		this.lakes = lumenLakes(shared.world, this.sample);
		this.model = new FaunaModel(shared.world.seed || shared.seed || 'nowhere', { sample: this.sample, lakes: this.lakes, avoid: p => this.avoid(p) });
	...
	update(dt) {
		...
		this.streamTimer -= dt;
		if (this.streamTimer <= 0 && (shared.caveAmount || 0) < 0.4) { this.stream(model.listener); this.streamTimer = 0.5; }
