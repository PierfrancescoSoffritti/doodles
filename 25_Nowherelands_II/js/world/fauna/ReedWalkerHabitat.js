...
export const REED_WORLD_GRAZE_SECONDS = 22;
...
export function reedHabitat(s, bankWater = s.water) {
 const level = Math.max(s.water,bankWater), height = s.ground-level;
 return Number.isFinite(height) && bankWater > .5 && height > .18 && height < 6 && !s.roof && (s.foam || 0) < .38 && (s.slope || 0) < .35;
}
export function reedSites(world, lakes = []) {
 ...
 for (const [ri, river] of (world.rivers || []).entries()) {
  ...
  for (let i = 2; i < river.count - 2; i += 12) {
   ...
   for (const side of [-1, 1]) {
    const across = (surfaceHalfWidth(d[o+RV.W],d[o+RV.D],d[o+RV.BANK],side,d[o+RV.BEND]) + 8) * side;
 ...
 for (const lake of lakes) for (const [i, shore] of lake.shore.entries()) {
  sites.push({ id: `lake:${lake.id}:${i}`, x: shore.x - shore.nx * 18, z: shore.z - shore.nz * 18, yaw: -Math.atan2(shore.tz, shore.tx), radius: 26, water: shore.y, form: shore.y > 140 ? 'tarn' : 'peat' });
 }
 return sites;
}
...
export function reedFootprint(traits, origin, yaw, sample, blocked = () => false, bankWater) {
 ...
 if (!reedHabitat(center,bankWater) || blocked(origin.x, origin.z, 3 * scale, center.ground)) return null;
 ...
 if (feet.some(f => !Number.isFinite(f.y) || f.surface.roof || f.surface.slope > .5 || f.surface.ground < Math.max(f.surface.water,bankWater) + .05 || blocked(f.x, f.z, .4 * scale, f.surface.ground))) return null;
 const localFeet = feet.map(f => reedLocalPoint(origin, yaw, f, scale));
 // On land the underside grazes the bank vegetation, rather than sinking
 // below the soil to reach a river surface that is lower than its feet.
 const feedingHeight = center.ground + .12 * scale;
...
export function createReedFamily(site, seed, sample, blocked) {
 ...
 for (const definition of family) {
  ...
  for (let i = 0; i < 90; i++) {
   ...
   const surface = sample(p.x, p.z); if (!reedHabitat(surface,site.water)) continue;
