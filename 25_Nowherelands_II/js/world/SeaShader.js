...
// shore: crest lines parallel to the coast, growing as they shoal, breaking over the last few
...
export const seaWaveGlsl = /* glsl */`
	...
	// the ocean swell gives way to the breakers over the last tens of metres
	float deepEnv(float d) { return smoothstep(2.0, 40.0, d); }
...
export function seaFragmentShader(shared) {
	...
	void main() {
		...
		float crest = clamp((vWorldPos.y - uWaterLevel) / 1.3, 0.0, 1.0);
		...
		float fres = 0.04 + 0.55 * pow(1.0 - NdV, 3.0);
