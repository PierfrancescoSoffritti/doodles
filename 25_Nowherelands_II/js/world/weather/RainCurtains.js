...
export class RainCurtains {
		...
		this.mesh=new THREE.InstancedMesh(g,new THREE.ShaderMaterial({
			...
			fragmentShader: /* glsl */`
				...
				void main(){
					...
					float vertical=smoothstep(0.0,0.045,vUv.y)*(1.0-smoothstep(0.78,1.0,vUv.y));
					...
					float alpha=edge*vertical*min(1.0, dot(weatherPrecipitation(vWorld), vec3(1.0)))*streak*mix(0.17,0.32,w.b)*smoothstep(150.0,500.0,dist)*(1.0-smoothstep(10000.0,16000.0,dist));
					vec3 tone=mix(vec3(0.22,0.20,0.32),vec3(0.085,0.07,0.13),w.b);
					tone=mix(tone,vec3(0.10,0.12,0.16),weatherSnow(vWorld));
					gl_FragColor=vec4(tone+uLightning*0.35,alpha);
				}`,
		}),64);
		this.mesh.renderOrder=2;this.mesh.count=0;this.mesh.frustumCulled=false;scene.add(this.mesh);
	...
	update(position,base) {
		...
		for(let i=0;i<this.centers.length;i++){
			...
			d.scale.set(m.cell*7,height,1);d.updateMatrix();this.mesh.setMatrixAt(i,d.matrix);
		}
