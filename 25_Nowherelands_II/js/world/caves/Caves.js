...
vec3 caveLight(vec3 base,vec3 p,vec3 n){
 vec3 toLight=uCamera-p;float d=length(toLight);vec3 l=toLight/max(d,.01);
 float lamp=uLamp*(.22+.78*max(dot(n,l),0.0))/(1.0+d*d*.0013);
 float sky=daylight(p)*(.3+.4*max(n.y,0.0));
