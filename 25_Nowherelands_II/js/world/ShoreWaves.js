...
export const shoreWaveGlsl = /* glsl */`
	...
	float cliffReach(vec2 p, float t) {
		return (3.0 + 8.0 * clamp(uSurfEnergy - 1.0, 0.0, 3.0)) * (0.65 + 0.35 * shoreSet(p, t));
	}
	...
	float shoreLift(float d, vec2 p, float t) {
		...
		return 1.1 * clamp(uSurfEnergy, 1.0, 4.0) * shoreEnv(d) * shoreSet(p, t) * (2.0 * pow(c, 1.7) - 0.85);
	}
	...
	float shoreSlope(float d, vec2 p, float t) {
		...
		return 1.1 * clamp(uSurfEnergy, 1.0, 4.0) * shoreEnv(d) * shoreSet(p, t) * 1.7 * pow(c, 0.7) * cos(ph) * SHORE_K;
	}
	...
	float shoreRunUp(vec2 p, float t) {
		...
		return 7.5 * sqrt(clamp(uSurfEnergy, 1.0, 4.0)) * shoreSet(p, t) * f;
	}
